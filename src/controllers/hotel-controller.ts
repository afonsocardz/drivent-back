...
export async function getCapacityHotel(req: AuthenticatedRequest, res: Response) {
  const { hotelId } = req.params;

  try {
    const vacancies = await hotelService.getCapacity(Number(hotelId));
    const data = { hotelId, vacancies };
